...
const CourseProgress = () => {
  ...
  const { data, isLoading, isError, refetch, error } = useGetCourseProgressQuery(courseId);

  const [updateLectureProgress] = useUpdateLectureProgressMutation();
  const [completeCourse, { data: markCompleteData, isSuccess: completedSuccess, isError: completeError }] = useCompleteCourseMutation();
  const [inCompleteCourse, { data: markInCompleteData, isSuccess: inCompletedSuccess, isError: inCompleteError }] = useInCompleteCourseMutation();
  ...
  useEffect(() => {
    ...
    if (inCompletedSuccess && markInCompleteData?.message) {
      toast.success(markInCompleteData.message);
      refetch();
    } else if (inCompleteError) {
      toast.error("Failed to mark as incomplete.");
    }
  }, [completedSuccess, inCompletedSuccess, completeError, inCompleteError]);

  if (isLoading) return <p>Loading...</p>;
  if (isError || !data?.data || !data.data.courseDetails) return <p>Failed to load course details.</p>;
  ...
  const handleCompleteCourse = async () => {
    try {
      ...
      toast.error("Could not complete course.");
    }
  };
  ...
  const handleInCompleteCourse = async () => {
    try {
      ...
      toast.error("Could not mark course as incomplete.");
    }
  };
        ...
        <div className="flex-1 md:w-3/5 h-fit rounded-lg shadow-lg p-4">
          {initialLecture ? (
            <>
              ...
              <div className="mt-2">
                <h3 className="font-medium text-lg">
                  Lecture {lectures.findIndex((lec) => lec._id === initialLecture._id) + 1}:{" "}
                  {initialLecture?.lectureTitle || "Untitled Lecture"}
                </h3>
              </div>
            </>
          ) : (
